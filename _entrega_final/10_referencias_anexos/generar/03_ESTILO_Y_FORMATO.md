...
- **Nada de listas donde corresponde prosa.** Las referencias son una lista numerada en formato IEEE. Los anexos son evidencia documentada: tablas, manifiestos y figuras con su procedencia, con la prosa mínima necesaria para saber qué se está mirando.
...
- Título **debajo** de la tabla, en cursiva, numerado: *Tabla E.1. Tablero de las diez hipótesis pre-registradas.*
...
- Toda tabla debe estar **referenciada desde el texto** antes de aparecer: «…se resume en la
  Tabla E.1». Una tabla que nadie menciona es una tabla que el comité pregunta por qué está.
...
- Para las que aún no existen, usa exactamente el marcador `[FIGURA PENDIENTE E.N]` seguido de su pie. No
...
Anexo E. Evidencia de la campaña…              (nivel 1)
E.1. Pre-registro de hipótesis                 (nivel 2)
```

## Referencias cruzadas

- A otras secciones: «§6.8», «el Capítulo VI», «§4.2.4».
- **Formato IEEE numérico**, numeración por orden de primera aparición en el cuerpo.
- Cada entrada nueva se marca `[REF-NUEVA-n]` en el texto y se lista aparte: la renumeración final
  se hace en Word con referencias cruzadas, **nunca a mano**.
- Las fuentes con DOI lo llevan. Las fuentes en línea llevan fecha de último acceso.
...
- ❌ **Referencias inventadas.** Es la prohibición absoluta de este bloque. Si no tienes la cita
  exacta, escribe `[CITA PENDIENTE: descripción precisa de lo que hay que buscar]`.
- ❌ Wikipedia, blogs sin autoría reconocida, foros. El manual los prohíbe explícitamente.
- ❌ Datos personales de mascota, propietario o clínica en cualquier fichero anexado.

Una referencia inventada es el único error de este documento que un miembro del comité puede
verificar desde su teléfono en treinta segundos, en mitad de la defensa. No hay ningún atajo que
compense ese riesgo.
