...
- **Nada de listas donde corresponde prosa.** La metodología es prosa procedimental. Las listas se reservan para enumeraciones genuinas: las quince preguntas del protocolo, las tres limitaciones de la ablación, los parámetros de un diseño experimental.
...
- Título **debajo** de la tabla, en cursiva, numerado: *Tabla 3.12. Identidad sellada del runtime bajo el que se midió.*
...
- Toda tabla debe estar **referenciada desde el texto** antes de aparecer: «…se resume en la
  Tabla 3.12». Una tabla que nadie menciona es una tabla que el comité pregunta por qué está.
...
- Para las que aún no existen, usa exactamente el marcador `[FIGURA PENDIENTE 3.N]` seguido de su pie. No
...
```
Capítulo III — Metodología                     (nivel 1)
3.11. Metodología de recaracterización…        (nivel 2)
3.11.2. Auditoría de comparabilidad previa     (nivel 3)
```

## Referencias cruzadas

- A otras secciones: «§6.8», «el Capítulo VI», «§4.2.4».
- Cuando un procedimiento produzca un resultado, **remite** en lugar de adelantarlo: «el resultado
  de este contraste se presenta en §6.8».
- Los métodos estadísticos nuevos (Wilson, McNemar, Wilcoxon, potencia) necesitan cita. Marca
  `[CITA PENDIENTE: …]` si no la tienes.
...
- ❌ **Resultados.** Es la prohibición central de este capítulo: aquí va *cómo se midió*, no *qué
  salió*. «Se contrastó con Wilcoxon pareado por caso» sí; «la latencia bajó un 60,6 %» no.
- ❌ Interpretación de lo medido.
- ❌ Recomendaciones.

La prueba de altitud de este capítulo es sencilla: si borras el Capítulo VI, §3.11 debe seguir
leyéndose completa y con sentido. Si al hacerlo se queda coja, es que metiste resultados.
