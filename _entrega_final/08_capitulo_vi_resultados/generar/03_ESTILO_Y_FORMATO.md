...
- **Nada de listas donde corresponde prosa.** Los resultados se presentan en prosa con tablas y figuras de apoyo. Cada tabla va precedida de una frase que dice qué mirar en ella y seguida de una que dice qué se lee. Una tabla sola, sin lectura, no es un resultado presentado.
...
- Título **debajo** de la tabla, en cursiva, numerado: *Tabla 6.18. Réplica estricta pareada L4 → A100.*
...
- Toda tabla debe estar **referenciada desde el texto** antes de aparecer: «…se resume en la
  Tabla 6.18». Una tabla que nadie menciona es una tabla que el comité pregunta por qué está.
...
```
Capítulo VI — Análisis de los resultados       (nivel 1)
6.8. Recaracterización del runtime…            (nivel 2)
6.8.3. Ablación de la decodificación…          (nivel 3)
```

## Referencias cruzadas

- A otras secciones: «§6.8», «el Capítulo VI», «§4.2.4».
- Toda proporción se acompaña de su **intervalo de confianza de Wilson**, incluidas las observadas
  en cero. Un cero sin intervalo no es un resultado.
- Remite al Capítulo III para el método: «el diseño de esta medición se describe en §3.11».
- Remite al Capítulo VII para lo que haya que hacer con un hallazgo. Aquí no.
...
- ❌ **Conclusiones y sugerencias.** El manual (p. 13) es literal: el apartado de resultados «no
  incluye conclusiones ni sugerencias». Nada de «se recomienda», «conviene», «debería».
- ❌ Comparaciones de rendimiento físico entre unidades de procesamiento gráfico: la configuración
  anterior **no es verificable** y toda cifra de decodificación, MBU o TPOT es caracterización
  absoluta de la A100.
- ❌ Leer una bajada de fallos como corrección de los mismos fallos. Ver el hecho 7.2.

Este capítulo es el que sostiene el proyecto. La honestidad de sus intervalos y de sus «no
concluyente» es lo que hace creíbles sus cifras buenas: un capítulo que solo publica lo que salió
bien no se distingue de uno que no midió.
